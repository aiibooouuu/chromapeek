  ...
  return (
    <div className="text-extractor">
      <div className="text-header">
        ...
        <button className="copy-btn" onClick={copyText} disabled={!text}>
          <FaRegCopy />
          Copy
        </button>
      </div>
      ...
      {text ? (
        <textarea className="text-output" value={text} readOnly />
      ) : (
        <div className="text-empty">
          <FaFileLines size={44} />
          <h3>No Text Available</h3>
          <p>Scan the current page to extract readable text content.</p>
        </div>
      )}
    </div>
  );
